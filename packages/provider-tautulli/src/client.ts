...
export class TautulliClient {
  ...
  private async command<T>(
    ...
    try {
      ...
      if (res.status === 401)
        throw new TautulliApiError("auth_failed", "Invalid API key");
      if (res.status === 404)
        throw new TautulliApiError("not_found", `Not found: ${cmd}`);
      if (!res.ok)
        throw new TautulliApiError("api_error", `HTTP ${res.status}`);
      const json = (await res.json()) as {
        response: { result: string; message?: string; data: T };
      };
      if (json.response.result !== "success") {
        throw new TautulliApiError(
          "api_error",
          json.response.message ?? "API returned failure",
        );
      }
