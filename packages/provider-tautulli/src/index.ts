...
import { TautulliClient, TautulliApiError } from "./client.ts";
...
class TautulliProviderInstance {
	...
	constructor(config: { url: string; api_key: string; timeoutMs: number }) {
		this.client = new TautulliClient(config.url, config.api_key, config.timeoutMs);
	}
	...
	async check(target: unknown, checks: string[]): Promise<CheckResult[]> {
		...
		switch (t.type) {
			case "activity": return this.checkActivity(checks);
			case "history": return this.checkHistory(checks);
	...
	private async checkActivity(checks: string[]): Promise<CheckResult[]> {
		...
		return checks.map((check) => {
			switch (check) {
				case "streams": return { check, value: data.sessions.length };
				default: return {
					check,
	...
	private async checkHistory(checks: string[]): Promise<CheckResult[]> {
		...
		return checks.map((check) => {
			switch (check) {
				case "plays": return { check, value: data.recordsTotal };
				default: return {
					check,
					error: { level: "check" as const, code: "unknown_check", title: `Unknown check: ${check}`, message: `The check "${check}" is not supported on this target type` },
				};
...
const tautulliRemote = {
	...
	providerConfigSchema: providerConfigSchema(tautulliProvider.config, tautulliProvider.targetTypes, tautulliProvider.name),
	targetConfigSchema: allTargetConfigsSchema(tautulliProvider.targetTypes),
	createInstance: (config: unknown) => new TautulliProviderInstance(makeInstance(config)),
} satisfies Provider;
