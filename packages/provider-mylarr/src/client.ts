...
export class MylarrClient {
	...
	private async request<T>(cmd: string, params: Record<string, string> = {}): Promise<T> {
		...
		try {
			...
			if (typeof json === "object" && json !== null && "success" in json) {
				const envelope = json as { success: boolean; data?: T; error?: { code: number; message: string } };
				if (!envelope.success) {
	...
	async getVersion(): Promise<{ current_version: string; latest_version: string; commits_behind: number }> {
		return this.request("getVersion");
	}

	async getComic(id: string): Promise<{
		comic: Array<{ id: string; name: string; status: string; totalIssues: number }>;
	...
	async getWanted(): Promise<{
		issues: Array<{ ComicName: string; Issue_Number: string; ComicID: string; IssueID: string }>;
