...
import { Pipeline, type OutboxConsumer } from "./outbox-consumer.ts";

const DEFAULT_MAX_UNACKED = 10;
const RECONNECT_MS = 2000;

function dispatch(instance: ChannelInstance, payload: ChannelOutboxPayload): void {
	switch (payload.type) {
...
class WebSocketConsumer implements OutboxConsumer {
	...
	private connect(): void {
		...
		ws.onopen = () => {
			ws.send(JSON.stringify({ type: "channel_hello", channelId: this.workerId }));
		};
...
export async function startChannelWorker(
	...
	while ((entry = await pipeline.next()) !== null) {
