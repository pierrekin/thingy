...
export function getBucketBounds(time: Date, config: BucketConfig): { start: number; end: number } {
	const timeMs = time.getTime();
	const bucketStart = Math.floor(timeMs / config.durationMs) * config.durationMs;
	const bucketEnd = bucketStart + config.durationMs;
	return { start: bucketStart, end: bucketEnd };
}

export function getTimeRangeBounds(config: BucketConfig): { start: number; end: number } {
	const now = Date.now();
	const currentBucketStart = Math.floor(now / config.durationMs) * config.durationMs;
