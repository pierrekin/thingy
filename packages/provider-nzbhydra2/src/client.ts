...
export class NzbHydra2Client {
	...
	private async post<T>(path: string, body: Record<string, unknown>): Promise<T> {
		...
		try {
			...
			if (res.status === 404) throw new NzbHydra2ApiError("not_found", `Not found: ${path}`);
			if (!res.ok) throw new NzbHydra2ApiError("api_error", `HTTP ${res.status}`);
	...
	async getIndexerStatuses(): Promise<IndexerStatus[]> {
		return this.post<IndexerStatus[]>("/api/stats/indexers", { apikey: this.apiKey });
	}

	async getSearchCount(): Promise<number> {
		const data = await this.post<{ totalElements: number }>("/api/history/searches", {
			apikey: this.apiKey,
			request: { sortModel: { column: "time", sortMode: -1 }, filterModel: {} },
		});
		return data.totalElements;
	}

	async getDownloadCount(): Promise<number> {
		const data = await this.post<{ totalElements: number }>("/api/history/downloads", {
			apikey: this.apiKey,
			request: { sortModel: { column: "time", sortMode: -1 }, filterModel: {} },
		});
		return data.totalElements;
