import type { AgentConfig, Provider, ProviderDefinition } from "mantle-framework";
...
export function getProviderDefinition(providerType: string): ProviderDefinition | undefined {
	return registry[providerType]?.definition;
}

export function getProviderType(instanceName: string, instanceConfig: unknown): string {
	if (
		instanceConfig &&
