...
import { ServarrClientV1, ServarrClientV3, ServarrApiError } from "./client.ts";
...
function targetError(checks: string[], err: ServarrApiError): CheckResult[] {
	return checks.map((check) => ({
		check,
		error: { level: "target" as const, code: err.code, title: errorTitle(err.code), message: err.message },
	}));
}
...
async function checkHealth(client: ServarrClientV1 | ServarrClientV3, checks: string[]): Promise<CheckResult[]> {
	...
	return checks.map((check) => {
		switch (check) {
			case "errors": return { check, value: errors };
			case "warnings": return { check, value: warnings };
...
async function checkQueue(client: ServarrClientV1 | ServarrClientV3, checks: string[]): Promise<CheckResult[]> {
	...
	const errors = queue.records.filter((r) => r.status === "failed" || r.errorMessage !== undefined).length;
	return checks.map((check) => {
		switch (check) {
			case "size": return { check, value: size };
			case "errors": return { check, value: errors };
			default: return { check, error: { level: "check" as const, code: "unknown_check", title: `Unknown check: ${check}`, message: `The check "${check}" is not supported on this target type` } };
		}
	});
...
class RadarrProviderInstance {
	...
	async check(target: unknown, checks: string[]): Promise<CheckResult[]> {
		...
		switch (t.type) {
			case "movie": return this.checkMovie(t.movieId!, checks);
			case "queue": return checkQueue(this.client, checks);
			case "health": return checkHealth(this.client, checks);
			default: throw new Error(`Unknown target type: ${t.type}`);
		}
	}
	private async checkMovie(id: number, checks: string[]): Promise<CheckResult[]> {
		...
		return checks.map((check) => {
			switch (check) {
				case "monitored": return { check, value: movie.monitored ? 1 : 0 };
				case "downloaded": return { check, value: movie.hasFile ? 1 : 0 };
				default: return { check, error: { level: "check" as const, code: "unknown_check", title: `Unknown check: ${check}`, message: `The check "${check}" is not supported on this target type` } };
			}
		});
	}
...
class LidarrProviderInstance {
	...
	constructor(private config: { url: string; api_key: string; timeoutMs: number }) {
		this.client = new ServarrClientV1(config.url, config.api_key, config.timeoutMs);
	}
	getErrorTitle(code: string): string { return errorTitle(code); }
	async check(target: unknown, checks: string[]): Promise<CheckResult[]> {
		const t = target as { type: string; artistId?: number };
		switch (t.type) {
			case "artist": return this.checkArtist(t.artistId!, checks);
			case "queue": return checkQueue(this.client, checks);
			case "health": return checkHealth(this.client, checks);
			default: throw new Error(`Unknown target type: ${t.type}`);
		}
	}
	private async checkArtist(id: number, checks: string[]): Promise<CheckResult[]> {
		...
		const missing = artist.statistics.totalTrackCount - artist.statistics.trackFileCount;
		return checks.map((check) => {
			switch (check) {
				case "monitored": return { check, value: artist.monitored ? 1 : 0 };
				case "missing_tracks": return { check, value: Math.max(0, missing) };
				default: return { check, error: { level: "check" as const, code: "unknown_check", title: `Unknown check: ${check}`, message: `The check "${check}" is not supported on this target type` } };
			}
		});
	}
...
class ProwlarrProviderInstance {
	...
	async check(target: unknown, checks: string[]): Promise<CheckResult[]> {
		...
		switch (t.type) {
			case "indexer": return this.checkIndexer(t.indexerId!, checks);
			case "health": return checkHealth(this.client, checks);
			default: throw new Error(`Unknown target type: ${t.type}`);
		}
	}
	private async checkIndexer(id: number, checks: string[]): Promise<CheckResult[]> {
		...
		return checks.map((check) => {
			switch (check) {
				case "enabled": return { check, value: indexer.enableRss ? 1 : 0 };
				default: return { check, error: { level: "check" as const, code: "unknown_check", title: `Unknown check: ${check}`, message: `The check "${check}" is not supported on this target type` } };
			}
		});
	}
...
class WhisparrProviderInstance {
	...
	async check(target: unknown, checks: string[]): Promise<CheckResult[]> {
		...
		switch (t.type) {
			case "scene": return this.checkScene(t.sceneId!, checks);
			case "queue": return checkQueue(this.client, checks);
			case "health": return checkHealth(this.client, checks);
			default: throw new Error(`Unknown target type: ${t.type}`);
		}
	}
	private async checkScene(id: number, checks: string[]): Promise<CheckResult[]> {
		...
		return checks.map((check) => {
			switch (check) {
				case "monitored": return { check, value: scene.monitored ? 1 : 0 };
				case "downloaded": return { check, value: scene.hasFile ? 1 : 0 };
				default: return { check, error: { level: "check" as const, code: "unknown_check", title: `Unknown check: ${check}`, message: `The check "${check}" is not supported on this target type` } };
			}
		});
	}
...
const sonarrRemote = {
	...
	providerConfigSchema: providerConfigSchema(sonarrProvider.config, sonarrProvider.targetTypes, sonarrProvider.name),
	targetConfigSchema: allTargetConfigsSchema(sonarrProvider.targetTypes),
	createInstance: (config: unknown) => new SonarrProviderInstance(makeInstance(config)),
} satisfies Provider;

const radarrRemote = {
	name: radarrProvider.name,
	definition: radarrProvider,
	providerConfigSchema: providerConfigSchema(radarrProvider.config, radarrProvider.targetTypes, radarrProvider.name),
	targetConfigSchema: allTargetConfigsSchema(radarrProvider.targetTypes),
	createInstance: (config: unknown) => new RadarrProviderInstance(makeInstance(config)),
} satisfies Provider;

const lidarrRemote = {
	name: lidarrProvider.name,
	definition: lidarrProvider,
	providerConfigSchema: providerConfigSchema(lidarrProvider.config, lidarrProvider.targetTypes, lidarrProvider.name),
	targetConfigSchema: allTargetConfigsSchema(lidarrProvider.targetTypes),
	createInstance: (config: unknown) => new LidarrProviderInstance(makeInstance(config)),
} satisfies Provider;

const prowlarrRemote = {
	name: prowlarrProvider.name,
	definition: prowlarrProvider,
	providerConfigSchema: providerConfigSchema(prowlarrProvider.config, prowlarrProvider.targetTypes, prowlarrProvider.name),
...
const whisparrRemote = {
	...
	providerConfigSchema: providerConfigSchema(whisparrProvider.config, whisparrProvider.targetTypes, whisparrProvider.name),
	targetConfigSchema: allTargetConfigsSchema(whisparrProvider.targetTypes),
	createInstance: (config: unknown) => new WhisparrProviderInstance(makeInstance(config)),
} satisfies Provider;
