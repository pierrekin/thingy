...
import { MylarrClient, MylarrApiError } from "./client.ts";
...
class MylarrProviderInstance {
	...
	constructor(private config: { url: string; api_key: string; timeoutMs: number }) {
		this.client = new MylarrClient(config.url, config.api_key, config.timeoutMs);
	}
	...
	async check(target: unknown, checks: string[]): Promise<CheckResult[]> {
		...
		switch (t.type) {
			case "comic": return this.checkComic(t.comicId!, checks);
			case "wanted": return this.checkWanted(checks);
			default: throw new Error(`Unknown target type: ${t.type}`);
		}
	}

	private async checkComic(id: string, checks: string[]): Promise<CheckResult[]> {
		...
		if (result.comic.length === 0) {
			const notFound = new MylarrApiError("not_found", `No comic found with id: ${id}`);
			return targetError(checks, notFound);
		}
		const comic = result.comic[0]!;
		const downloaded = result.issues.filter((i) => i.status === "Downloaded").length;
		const missing = Math.max(0, comic.totalIssues - downloaded);
		return checks.map((check) => {
			switch (check) {
				case "monitored": return { check, value: comic.status === "Active" ? 1 : 0 };
				case "missing_issues": return { check, value: missing };
	...
	private async checkWanted(checks: string[]): Promise<CheckResult[]> {
		...
		return checks.map((check) => {
			switch (check) {
				case "count": return { check, value: count };
				default: return { check, error: { level: "check" as const, code: "unknown_check", title: `Unknown check: ${check}`, message: `The check "${check}" is not supported on this target type` } };
			}
		});
	}
...
const mylarrRemote = {
	...
	providerConfigSchema: providerConfigSchema(mylarrProvider.config, mylarrProvider.targetTypes, mylarrProvider.name),
	targetConfigSchema: allTargetConfigsSchema(mylarrProvider.targetTypes),
	createInstance: (config: unknown) => new MylarrProviderInstance(makeInstance(config)),
} satisfies Provider;
