...
import type { BucketConfig } from "./buckets.ts";
...
export class WebService {
  ...
  private async handleAuthenticate(
    ...
    ws.send(
      JSON.stringify({ type: "authentication_error", error: result.reason }),
    );
    ws.close();
  }
  ...
  private async handleStateSubscription(
    ...
    // Create subscription
    const subscription = new StateSubscription(
      req.id,
      ws,
      req.start,
      req.end,
    ...
    if (req.snapshot) {
      ws.send(
        JSON.stringify({ type: "snapshot_complete", subscriptionId: req.id }),
      );
  ...
  private async handleMetricsSubscription(
    ...
    if (!req.snapshot) {
      ...
      const unsub = this.outcomePublisher.subscribe(async (outcome) => {
        if (
          !subscription.matches(outcome.provider, outcome.target, outcome.check)
        )
          return;
        if (!outcome.success) return;

        const bucketStart =
          Math.floor(outcome.time / subscription.bucketDurationMs) *
        ...
        invariant(
          bucket,
          `metrics aggregation returned empty for just-written outcome at ${outcome.time} on ${outcome.provider}/${outcome.target}/${outcome.check}`,
        );
  ...
  private async sendMetricsData(
    ...
    const expectedCount = Math.ceil(
      (endTime - start) / subscription.bucketDurationMs,
    );
  ...
  private fillMetricsBuckets(
    ...
    const lookup = new Map<
      number,
      { bucketStart: number; bucketEnd: number; mean: number | null }
    >();
  ...
  private async sendInitialData(
    ...
    const providerBuckets = this.fillProviderBuckets(
      storedBuckets.providers,
      start,
      config,
    );
    const targetBuckets = this.fillTargetBuckets(
      storedBuckets.targets,
      start,
      config,
    );
    const checkBuckets = this.fillCheckBuckets(
      storedBuckets.checks,
      start,
      config,
    );
    ...
    const latestProviderStatuses =
      await this.outcomeStore.getLatestProviderStatuses();
    ...
    if (this.channelStores) {
      const channelBuckets =
        await this.channelStores.bucketStore.getChannelBuckets(start, endTime);
      const filledChannelBuckets = this.fillChannelBuckets(
        channelBuckets,
        start,
        config,
      );
      ...
      const channelEvents =
        await this.channelStores.eventStore.getChannelEventsInRange(
          start,
          endTime,
        );
      ...
      const latestChannelStatuses =
        await this.channelStores.outcomeStore.getLatestChannelStatuses();
    ...
    if (this.agentStores) {
      const agentBuckets = await this.agentStores.bucketStore.getAgentBuckets(
        start,
        endTime,
      );
      const filledAgentBuckets = this.fillAgentBuckets(
        agentBuckets,
        start,
        config,
      );
      ...
      const agentEvents =
        await this.agentStores.eventStore.getAgentEventsInRange(start, endTime);
      ...
      const latestAgentStatuses =
        await this.agentStores.outcomeStore.getLatestAgentStatuses();
  ...
  private subscribeToRealTimeUpdates(subscription: StateSubscription): void {
    ...
    const unsubProvider = this.bucketPublishers.provider.subscribe(
      (bucketMsg) => {
    ...
    const unsubProviderEvent = this.eventPublishers.provider.subscribe(
      (event) => {
    ...
    const unsubProviderStatus = this.providerStatusPublisher.subscribe(
      (update) => {
        const msg: ProviderStatusMessage = {
          type: "provider_status",
          subscriptionId: subscription.id,
    ...
    if (this.channelPublishers) {
      const unsubChannelBucket = this.channelPublishers.bucket.subscribe(
        (bucketMsg) => {
      ...
      const unsubChannelEvent = this.channelPublishers.event.subscribe(
        (event) => {
    ...
    if (this.channelStatusPublisher) {
      const unsubChannelStatus = this.channelStatusPublisher.subscribe(
        (update) => {
          const msg: ChannelStatusMessage = {
            type: "channel_status",
            subscriptionId: subscription.id,
    ...
    if (this.agentPublishers) {
      const unsubAgentBucket = this.agentPublishers.bucket.subscribe(
        (bucketMsg) => {
          if (!subscription.isInRange(bucketMsg.bucketStart)) return;
          const msg: AgentBucketMessage = {
            type: "agent_bucket",
    ...
    if (this.agentInstancesPublisher) {
      const unsubAgentInstances = this.agentInstancesPublisher.subscribe(
        (update) => {
          const msg: AgentInstancesMessage = {
            type: "agent_instances",
            subscriptionId: subscription.id,
  ...
  private async handleEventSubscription(
    ...
    const subscription = new EventDetailSubscription(
      req.id,
      ws,
      req.eventId,
      req.eventLevel,
    ...
    const outcomes = await this.outcomeStore.getOutcomesForEvent(
      req.eventId,
      req.eventLevel,
    );
  ...
  private handleUnsubscribe(
    _ws: MantleSocket<WebSocketData>,
  ...
  private fillCheckBuckets(
    ...
    for (const entity of entities) {
      const [provider, target, check] = entity.split("/") as [
        string,
        string,
        string,
      ];
