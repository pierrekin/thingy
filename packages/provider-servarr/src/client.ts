...
abstract class ServarrBaseClient {
  ...
  protected async request<T>(path: string): Promise<T> {
    ...
    try {
      ...
      if (res.status === 401)
        throw new ServarrApiError("auth_failed", "Invalid API key");
      if (res.status === 404)
        throw new ServarrApiError("not_found", `Not found: ${path}`);
...
export class ServarrClientV3 extends ServarrBaseClient {
  ...
  async getQueue(): Promise<{
    totalRecords: number;
    records: { status: string; errorMessage?: string }[];
  }> {
    return this.request("/api/v3/queue");
  }

  async getSeries(id: number): Promise<{
    monitored: boolean;
  ...
  async getMovie(
    id: number,
  ): Promise<{ monitored: boolean; hasFile: boolean }> {
...
export class ServarrClientV1 extends ServarrBaseClient {
  ...
  async getArtist(id: number): Promise<{
    monitored: boolean;
  ...
  async getIndexer(
    id: number,
  ): Promise<{ enableRss: boolean; enableAutomaticSearch: boolean }> {
