...
const NZBHYDRA2_API_KEY =
  process.env.NZBHYDRA2_API_KEY ?? "442HS2R934QOGQBGIII34TRK2P";
...
describe("provider: checks", () => {
  const nzbhydra2 = providers.find(
    (p) => p.name === "@mantle/nzbhydra2/remote",
  );
  invariant(nzbhydra2, "provider @mantle/nzbhydra2/remote not registered");
  const instance = nzbhydra2.createInstance({
    url: NZBHYDRA2_URL,
    api_key: NZBHYDRA2_API_KEY,
  });

  test("instance.total_searches returns a non-negative count", async () => {
    const [result] = await instance.check({ type: "instance" }, [
      "total_searches",
    ]);
  ...
  test("instance.disabled_indexers returns a non-negative count", async () => {
    ...
    expect(expectSuccess(result, "disabled_indexers")).toBeGreaterThanOrEqual(
      0,
    );
  });
});
