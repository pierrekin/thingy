...
# Check for uncommitted changes in this provider's dir
changed=$(git diff --name-only -- "$dir")
if [[ -z "$changed" ]]; then
...
git add $changed
git commit -m "feat($provider): add compatibility for $latest_version"
git push -u origin "$branch_name"

gh pr create \
  --title "feat($provider): add compatibility for $latest_version" \
  --body "$(cat <<EOF
