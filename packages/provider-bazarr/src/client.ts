...
export class BazarrClient {
	...
	private async request<T>(path: string): Promise<T> {
		...
		try {
			...
			if (res.status === 401) throw new BazarrApiError("auth_failed", "Invalid API key");
			if (res.status === 404) throw new BazarrApiError("not_found", `Not found: ${path}`);
	...
	async getStatus(): Promise<{ bazarr_version: string }> {
		const res = await this.request<{ data: { bazarr_version: string } }>("/api/system/status");
		return res.data;
	}

	async getHealth(): Promise<Array<{ object: string; issue: string }>> {
		const res = await this.request<{ data: Array<{ object: string; issue: string }> }>("/api/system/health");
		return res.data;
	}
