import type { OutboxStore, OutboxEntry } from "mantle-store";
...
class ChannelSession {
	...
	private async tick(): Promise<void> {
		...
		if (entries.length > 0) {
			this.sentCursor = entries[entries.length - 1]!.id;
...
export class ChannelSessionManager {
	...
	async handleHello(ws: MantleSocket<unknown>, channelId: string): Promise<void> {
