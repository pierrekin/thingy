...
const CADDY_PROXY_HEALTHY = process.env.CADDY_PROXY_HEALTHY ?? "http://localhost:8080";
const CADDY_PROXY_DEAD = process.env.CADDY_PROXY_DEAD ?? "http://localhost:8081";
const CADDY_PROXY_SLOW = process.env.CADDY_PROXY_SLOW ?? "http://localhost:8082";
...
describe("provider: checks", () => {
	...
	test("server.config_reload_success returns 1", async () => {
		const [result] = await instance.check({ type: "server" }, ["config_reload_success"]);
	...
	test("upstream(healthy).fails is 0 and num_requests is 0 at idle", async () => {
		const results = await instance.check({ type: "upstream", address: "upstream:80" }, ["num_requests", "fails"]);
	...
	test("upstream(healthy).fails stays 0 after successful requests", async () => {
		...
		const [result] = await instance.check({ type: "upstream", address: "upstream:80" }, ["fails"]);
	...
	test("upstream(slow).num_requests is 1 while in-flight, 0 after completion", async () => {
		...
		await Bun.sleep(500);
		const [inflight] = await instance.check({ type: "upstream", address: "slow-upstream:80" }, ["num_requests"]);
		expect(expectSuccess(inflight, "num_requests")).toBe(1);
		await fetchPromise;
		const [done] = await instance.check({ type: "upstream", address: "slow-upstream:80" }, ["num_requests"]);
	...
	test("upstream(dead).fails increments after failed requests", async () => {
		...
		const [result] = await instance.check({ type: "upstream", address: "127.0.0.1:9999" }, ["fails"]);
