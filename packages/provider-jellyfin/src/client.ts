...
export class JellyfinClient {
  ...
  private async request<T>(path: string): Promise<T> {
    ...
    try {
      ...
      if (res.status === 401)
        throw new JellyfinApiError("auth_failed", "Invalid or expired token");
      if (res.status === 403)
        throw new JellyfinApiError("auth_failed", "Insufficient permissions");
      if (res.status === 404)
        throw new JellyfinApiError("not_found", `Not found: ${path}`);
      if (!res.ok)
        throw new JellyfinApiError("api_error", `HTTP ${res.status}`);
