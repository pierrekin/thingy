...
import { z } from "zod";
import { JellyfinApiError, JellyfinClient } from "./client.ts";
...
class JellyfinProviderInstance {
  ...
  private async checkHealth(checks: string[]): Promise<CheckResult[]> {
    ...
    return checks.map((check) => {
      switch (check) {
        case "sessions":
          return { check, value: sessions.length };
        default:
          return {
            check,
            error: {
...
const jellyfinRemote = {
  ...
  providerConfigSchema: providerConfigSchema(
    jellyfinProvider.config,
    jellyfinProvider.targetTypes,
    jellyfinProvider.name,
  ),
  targetConfigSchema: allTargetConfigsSchema(jellyfinProvider.targetTypes),
  createInstance: (config: unknown) =>
    new JellyfinProviderInstance(makeInstance(config)),
} satisfies Provider;
