...
import { z } from "zod";
import { CaddyApiError, CaddyClient } from "./client.ts";
...
class CaddyProviderInstance {
  ...
  async check(target: unknown, checks: string[]): Promise<CheckResult[]> {
    ...
    switch (t.type) {
      case "server":
        return this.checkServer(checks);
      case "upstream":
        return this.checkUpstream(
          target as { type: string; address: string },
          checks,
        );
      default:
  ...
  private async checkServer(checks: string[]): Promise<CheckResult[]> {
    ...
    return checks.map((check) => {
      switch (check) {
        case "config_reload_success":
          return { check, value: reloadSuccess };
        default:
          return {
            check,
            error: {
  ...
  private async checkUpstream(
    target: { type: string; address: string },
    checks: string[],
  ): Promise<CheckResult[]> {
    ...
    if (!upstream) {
      return checks.map((check) => ({
        ...
        error: {
          ...
          title: errorTitle("upstream_not_found"),
          message: `No upstream with address "${target.address}" found`,
        },
      }));
    }
    ...
    return checks.map((check) => {
      switch (check) {
        case "num_requests":
          return { check, value: upstream.num_requests };
        case "fails":
          return { check, value: upstream.fails };
        default:
          return {
            check,
            error: {
...
function makeInstance(config: unknown): { url: string; timeoutMs: number } {
  ...
  return { url, timeoutMs: timeout ?? 30_000 };
}
