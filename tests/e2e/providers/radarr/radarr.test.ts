...
const RADARR_API_KEY = process.env.RADARR_API_KEY ?? "e2etestapikeyfortesting12345678";
...
describe("smoke: api", () => {
  ...
  test("system/status returns a version string", async () => {
    const status = await radarrGet<{ version: string }>("/api/v3/system/status");
  ...
  test("health returns an array of items with expected shape", async () => {
    const health = await radarrGet<{ source: string; type: string; message: string }[]>("/api/v3/health");
  ...
  test("diskspace returns entries with freeSpace and totalSpace", async () => {
    const disks = await radarrGet<{ freeSpace: number; totalSpace: number }[]>("/api/v3/diskspace");
  ...
  test("queue returns totalRecords and records array", async () => {
    const queue = await radarrGet<{ totalRecords: number; records: unknown[] }>("/api/v3/queue");
...
describe("provider: checks", () => {
  ...
  const instance = radarr.createInstance!({ url: RADARR_URL, api_key: RADARR_API_KEY });
