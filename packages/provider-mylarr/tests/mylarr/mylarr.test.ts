...
const MYLARR_API_KEY =
  process.env.MYLARR_API_KEY ?? "e2etestapikeyfortesting123456789";

async function mylarrGet<T>(
  cmd: string,
...
describe("smoke: api", () => {
  ...
  test("getIndex returns success with an array of comics", async () => {
    const result = await mylarrGet<{ success: boolean; data: unknown[] }>(
      "getIndex",
    );
    expect(result.success).toBe(true);
...
describe("provider: checks", () => {
  ...
  invariant(mylarr, "provider @mantle/mylarr/remote not registered");
  const instance = mylarr.createInstance({
    url: MYLARR_URL,
    api_key: MYLARR_API_KEY,
  });
