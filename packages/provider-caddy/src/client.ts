	...
	async getConfigReloadSuccess(): Promise<number> {
		...
		for (const line of text.split("\n")) {
			if (line.startsWith("caddy_config_last_reload_successful ")) {
				...
				if (isNaN(val)) throw new CaddyApiError("api_error", "caddy_config_last_reload_successful metric has non-numeric value");
				return val;
			}
		}
		throw new CaddyApiError("metric_absent", "caddy_config_last_reload_successful metric not present in /metrics");
	}
