...
import { z } from "zod";
import { BazarrApiError, BazarrClient } from "./client.ts";
...
class BazarrProviderInstance {
  ...
  async check(target: unknown, checks: string[]): Promise<CheckResult[]> {
    ...
    switch (t.type) {
      case "health":
        return this.checkHealth(checks);
      case "wanted_movies":
        return this.checkWantedMovies(checks);
      case "wanted_episodes":
        return this.checkWantedEpisodes(checks);
      default:
  ...
  private async checkWantedMovies(checks: string[]): Promise<CheckResult[]> {
    ...
    return checks.map((check) => {
      switch (check) {
        case "count":
          return { check, value: result.total };
        default:
          return {
            check,
            error: {
